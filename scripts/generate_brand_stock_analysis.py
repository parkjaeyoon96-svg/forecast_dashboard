...
def generate_js_file(clothing_data, acc_data, update_date, output_path):
    """
    JavaScript 파일 생성
    """
    now = datetime.now()
...
'''
    ...
    # JSON 파일도 별도로 저장 (디버깅/확인용)
    json_output_path = output_path.replace('.js', '.json')
    json_data = {
        "metadata": metadata,
    ...
    print(f"[출력] JSON 파일도 저장됨: {json_output_path}")
...
def main():
    ...
    if year_month_folders:
        latest_folder = sorted(year_month_folders)[-1]
        etc_folder = os.path.join(raw_base, latest_folder, 'ETC')
    else:
        etc_folder = os.path.join(raw_base, '202511', 'ETC')
    ...
    metadata = generate_js_file(clothing_data, acc_data, update_date, output_path)
